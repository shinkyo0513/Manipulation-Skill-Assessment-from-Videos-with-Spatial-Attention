...
from dataset.sugury_dataset import SuguryDataset, SuguryDataset_Pair
from dataset.sugury_dataset import get_flow_feature_dict, get_rgb_feature_dict
...
parser.add_argument("--dataset_name", type=str, default='All',
                    choices=['All', 'KnotTying', 'Suturing', 'NeedlePassing'])
...
#         self.fc_att = nn.Linear(self.x_size+self.rnn_top_size, self.rnn_att_size, bias=False)
#         self.rnn_att = nn.GRUCell(self.rnn_att_size, self.rnn_att_size)
...
#             rnn_att_in = self.fc_att(rnn_att_in)
...
def get_train_test_pairs_dict(annotation_dir, dataset_name, split_idx):
    train_pairs_dict = {}
    train_videos = set()
    train_csv = join(annotation_dir, dataset_name+'_train_' +
    ...
    test_pairs_dict = {}
    test_videos = set()
    test_csv = join(annotation_dir, dataset_name+'_val_' +
                    format(split_idx, '01d')+'.csv')
...
if __name__ == "__main__":
    ...
    dataset_name_list = ['KnotTying', 'NeedlePassing', 'Suturing'] if args.dataset_name=='All' else [args.dataset_name]
    ...
    for dataset_name in dataset_name_list:
        dataset_dir = join('../dataset', dataset_name, dataset_name+'_640x480')
    ...
    for split_idx in range(1, 5):
        ...
        num_seg = 25
        dataset_train = SuguryDataset_Pair('fusion', video_rgb_feature_dict, video_flow_feature_dict, 
                                                  train_pairs_dict, seg_sample=num_seg)
        dataloader_train = DataLoader(dataset_train, batch_size=16, shuffle=True)

        dataset_test = SuguryDataset('fusion', video_rgb_feature_dict, video_flow_feature_dict, 
                                              video_name_list, seg_sample=num_seg)
        dataloader_test = DataLoader(dataset_test, batch_size=1, shuffle=False)

        model_ins = read_model(args.model, args.feature_type, num_seg)

        save_label = f'Surgery_{args.dataset_name}/{args.model}/{split_idx:01d}'
        
        best_acc = 0.0
        if args.continue_train:
            ckpt_dir = join('checkpoints', save_label, 'best_checkpoint.pth.tar')
            if exists(checkpoint):
                checkpoint = torch.load(ckpt_dir)
                model_ins.load_state_dict(checkpoint['state_dict'])
                best_acc = checkpoint['best_acc']
                print("Start from previous checkpoint, with rank_cor: {:.4f}".format(checkpoint['best_acc']))
            else:
...
#                                      lr=1e-5, weight_decay=0, amsgrad=True)
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model_ins.parameters()), 
                                            lr=5e-4, momentum=0.9, weight_decay=1e-2)    #real l2 reg = weight_decay*lr
#         scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=2)
        ...
        for epoch in range(args.epoch_num):
            ...
            if epoch_acc >= best_acc:
                best_acc = epoch_acc
                save_best_result(dataloader_test, test_videos, model_ins, device, best_acc, save_label)
