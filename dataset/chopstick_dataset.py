...
def extract_save_rgb_feature (dataset_dir):
    ...
    for video_name in video_name_list:
        ...
        video_feature = []
        for idx in range(5, seq_len-5, 1):
...
def extract_save_flow_feature (dataset_dir):
    ...
    for video_name in video_name_list:
        ...
        video_feature = []
        for idx in range(0, seq_len-9, 1):
...
def get_train_test_pairs_dict(annotation_dir, split_idx):
    train_pairs_dict = {}
    train_videos = set()
    train_csv = join(annotation_dir, 'chopstick_using_train_' +
    ...
    test_pairs_dict = {}
    test_videos = set()
    test_csv = join(annotation_dir, 'chopstick_using_val_' +
                    format(split_idx, '01d')+'.csv')
...
class ChopstickDataset (Dataset):
    def __init__ (self, f_type, video_rgb_feature_dict, video_flow_feature_dict, video_name_list, seg_sample=None):
        ...
        self.dataset_dir = '../dataset/ChopstickUsing/ChopstickUsing_Stationary_800x450'
    ...
    def read_one_video (self, video_name):
        video_dir = join(self.dataset_dir, video_name)
        ...
        rand_idx_list = [num+5 for num in rand_idx_list]
        sample = {'name': video_name, 'video': video_tensor, 'sampled_index': rand_idx_list}
        return sample

class ChopstickDataset_Pair (Dataset):
    def __init__ (self, f_type, video_rgb_feature_dict, video_flow_feature_dict, pairs_dict, seg_sample=None):
        ...
        self.dataset_dir = '../dataset/MIT_Dive_Dataset/diving_samples_len_ori_800x450'
...
if __name__ == "__main__":
    extract_save_rgb_feature('../dataset/ChopstickUsing/ChopstickUsing_Stationary_800x450')
    extract_save_flow_feature('../dataset/ChopstickUsing/ChopstickUsing_Stationary_800x450')
#     get_rgb_feature_dict('../dataset/ChopstickUsing/ChopstickUsing_Stationary_800x450')
