...
from dataset.dough_dataset import DoughDataset, DoughDataset_Pair
from dataset.dough_dataset import get_flow_feature_dict, get_rgb_feature_dict
...
def get_train_test_pairs_dict (annotation_dir, split_idx):
    train_pairs_dict = {}
    train_videos = set()
    train_csv = join(annotation_dir, 'DoughRolling_train_'+format(split_idx, '01d')+'.csv')
    ...
    test_pairs_dict = {}
    test_videos = set()
    test_csv = join(annotation_dir, 'DoughRolling_val_'+format(split_idx, '01d')+'.csv')
...
if __name__ == "__main__":
    ...
    dataset_dir = '../dataset/DoughRolling/DoughRolling_600x450'
    annotation_dir = '../dataset/DoughRolling/DoughRolling_Annotation/splits'
    ...
    for split_idx in range(1, 5):
        ...
        num_seg = 25
        dataset_train = DoughDataset_Pair('fusion', video_rgb_feature_dict, video_flow_feature_dict, 
                                                  train_pairs_dict, seg_sample=num_seg)
        dataloader_train = DataLoader(dataset_train, batch_size=16, shuffle=True)

        dataset_test = DoughDataset('fusion', video_rgb_feature_dict, video_flow_feature_dict, 
                                              video_name_list, seg_sample=num_seg)
        dataloader_test = DataLoader(dataset_test, batch_size=1, shuffle=False)

        model_ins = read_model(args.model, args.feature_type, num_seg)

        save_label = f'DoughRolling/{args.model}/{split_idx:01d}'
